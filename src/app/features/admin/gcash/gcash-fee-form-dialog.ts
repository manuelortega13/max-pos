...
import { GcashFeeTier, GcashFeeTierUpsertRequest } from '../../../core/models';
import { GcashService } from '../../../core/services/gcash.service';
import { SettingsService } from '../../../core/services/settings.service';

export type GcashFeeFormMode = 'create' | 'edit';

export interface GcashFeeFormData {
  readonly mode: GcashFeeFormMode;
  readonly tier?: GcashFeeTier;
  /** Active tiers excluding `tier` (when editing). Drives the
   *  client-side overlap pre-check so the cashier sees the same
   *  message the server would return. */
  readonly otherActive: readonly GcashFeeTier[];
}

@Component({
  selector: 'app-gcash-fee-form-dialog',
  imports: [
    ReactiveFormsModule,
    MatDialogModule,
    MatButtonModule,
  ...
  template: `
    <h2 mat-dialog-title class="gtf__title">
      <mat-icon>{{ data.mode === 'edit' ? 'edit' : 'add' }}</mat-icon>
      {{ data.mode === 'edit' ? 'Edit fee tier' : 'Add fee tier' }}
    </h2>

    <form [formGroup]="form" (ngSubmit)="submit()">
      <mat-dialog-content class="gtf__content">
        <div class="gtf__row">
          <mat-form-field appearance="outline" class="gtf__field">
        ...
        <mat-form-field appearance="outline" class="gtf__full">
          <mat-label>Service fee</mat-label>
          <span matTextPrefix>{{ currencySymbol() }}&nbsp;</span>
          <input matInput type="number" min="0" step="0.01" formControlName="fee" />
          <mat-hint>
            Charged when a transaction falls in this range. Range is
            <strong>min ≤ amount &lt; max</strong>.
          </mat-hint>
        </mat-form-field>

        <mat-slide-toggle formControlName="active" class="gtf__active">
          Active — used by the cashier lookup
        </mat-slide-toggle>

        @if (error(); as message) {
          <div class="gtf__error" role="alert">
  ...
  styles: [
    `
      ...
      .gtf__field { width: 100%; }
      .gtf__full { width: 100%; }
      .gtf__active { margin-top: 0.5rem; }
      .gtf__error {
...
export class GcashFeeFormDialog {
  private readonly fb = inject(FormBuilder);
  private readonly gcashService = inject(GcashService);
  private readonly settingsService = inject(SettingsService);
  private readonly dialogRef = inject(MatDialogRef<GcashFeeFormDialog>);
  private readonly snackBar = inject(MatSnackBar);
  protected readonly data = inject<GcashFeeFormData>(MAT_DIALOG_DATA);
  ...
  protected submit(): void {
    ...
    this.error.set(null);
    const req: GcashFeeTierUpsertRequest = {
    ...
    const obs =
      this.data.mode === 'edit' && this.data.tier
        ? this.gcashService.updateTier(this.data.tier.id, req)
        : this.gcashService.createTier(req);
    obs.subscribe({
      next: () => {
        this.snackBar.open(
  ...
  private hydrate(t: GcashFeeTier): void {
